...
function spherical_to_cart() {
    [theta, phi, rho] = [camera_params.pos_sph.x, camera_params.pos_sph.y, camera_params.pos_sph.z]
    return createVector(rho*sin(phi)*cos(theta), rho*sin(phi)*sin(theta), rho*cos(phi))
}
...
function spawn_by_shape() {
     ...
     */
    let circ_calc = function () {
        return math.sqrt(math.square(h / w) * (math.square(w) - math.square(x_abs - x)))
    }
